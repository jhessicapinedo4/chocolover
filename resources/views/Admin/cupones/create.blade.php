        ...
        <form class="form-new-coupon form-style-1" method="POST" action="{{ route('cupones.store') }}">
            ...
            <fieldset class="name">
                ...
                <input class="flex-grow" type="text" placeholder="Código del Cupón" name="codigo_cupon"
                    value="{{ old('codigo_cupon') }}" required>
                @error('codigo_cupon')
            ...
            <fieldset class="name">
                ...
                <input class="flex-grow" type="number" step="0.01" min="0" max="100"
                    placeholder="Monto del Descuento (en %)" name="descuento" value="{{ old('descuento') }}" required>
                @error('descuento')
                    <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                @enderror
            </fieldset>
            ...
            <fieldset class="name">
                ...
                <input class="flex-grow" type="date" name="fecha_inicio" value="{{ old('fecha_inicio') }}" required>
                @error('fecha_inicio')
                    <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                @enderror
            </fieldset>
            ...
            <fieldset class="name">
                ...
                <input class="flex-grow" type="date" name="fecha_expiracion" value="{{ old('fecha_expiracion') }}"
                    required>
            ...
            <fieldset class="category">
                ...
                <div class="flex items-center gap-2">
                    <label for="estado" style="font-size: 15px; margin-rigth:5px">
                        <input type="checkbox" name="estado" value="1" {{ old('estado') ? 'checked' : '' }}>
                        Activo
                    </label>
                </div>
            </fieldset>
            ...
            <div class="bot">
                <div></div>
                <button class="tf-button w208" type="submit">Guardar Cupón</button>
            </div>
        </form>
    </div>
@endsection
